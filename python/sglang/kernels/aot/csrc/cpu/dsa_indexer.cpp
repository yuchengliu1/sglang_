...
//
// Fused CPU kernels mirroring the CUDA JIT kernels in
// python/sglang/jit_kernel/csrc/deepseek_v32/indexer_k.cuh (K) and
// python/sglang/jit_kernel/csrc/deepseek_v4/main_norm_rope.cuh (Q, the
// kRopeFirst=true/kHadamard=false specialization used by V3.2/DSA).
//
...
// here applies it -- matching the CUDA fused kernels, which never include
// it for this model family either.
//
// Fixed dtypes (no generic dispatch): k_input/q_input/weight/bias/
// cos_sin_cache/weight_raw are always bf16 (the indexer's wk_weights_proj /
// wq_b / k_norm / rotary_emb all use bf16 params_dtype on CPU when fusion
// is on; see Indexer.__init__ in dsa_indexer.py), and positions is always
// int64 (see forward_batch_info.py's compute_position_torch /
// _clamp_position_native, both of which always produce int64 on CPU).
...
// Returns the scale (matches CUDA fused kernels: plain amax/448, no ue8m0
// power-of-2 rounding -- scale_fmt is not consulted here, same as the CUDA
// fused K/Q kernels).
...
void fused_k_indexer_norm_rope_store_cpu(
  ...
  at::parallel_for(0, N, 0, [&](int64_t begin, int64_t end) {
    ...
    for (int64_t i = begin; i < end; ++i) {
      ...
      // Round every element through bf16 and back to fp32. The CUDA fused K
      // store kernel does this before quantizing so the fp8 scale matches
      // the un-fused (norm -> rope -> cast-to-bf16 -> act_quant) reference
      // path; see indexer_k.cuh's FusedKIndexerNormRopeStoreKernel comment
      // "Round through bf16 first". Not applied on the Q path, matching
      // CUDA's fused_q_indexer_rope_first_quant, which quantizes directly
      // from the fp32 accumulator.
      // {
      //   using fVec = Vectorized<float>;
      //   using bVec = Vectorized<at::BFloat16>;
      //   constexpr int64_t kStep = 2 * fVec::size();
      //   for (int64_t d = 0; d < kHeadDim; d += kStep) {
      //     fVec x0 = fVec::loadu(buf + d);
      //     fVec x1 = fVec::loadu(buf + d + fVec::size());
      //     bVec rounded = convert_from_float_ext<at::BFloat16>(x0, x1);
      //     std::tie(x0, x1) = at::vec::convert_to_float(rounded);
      //     x0.store(buf + d);
      //     x1.store(buf + d + fVec::size());
      //   }
      // }

      // out_cache_loc may still be int32 or int64 (unlike positions, which
      // is always int64 on CPU; see forward_batch_info.py).
...
// * fp8_scale[b, h], matching the CUDA fused_q_indexer_rope_first_quant
// kernel (see main_norm_rope.cuh, FusedQIndexerRopeHadamardQuantKernel with
