...
// largest values in `score_row`, unsorted, ties broken toward the smaller index (mirrors
// the CUDA fast_topk radix-select's ascending scan order). Only fills the first returned
// count of `out_local_indices`; the caller pads the remainder with -1.
