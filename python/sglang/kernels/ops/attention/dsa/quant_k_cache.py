...
def _quantize_k_cache_ref_separate(
    ...
    """Pure-PyTorch (device-agnostic, CPU-safe) counterpart of
    ``_quantize_k_cache_fast_separate``. Triton kernels cannot run on CPU
    tensors, so this mirrors the exact same per-tile ``max(abs)/448`` scale
    (no eps, no UE8M0 rounding -- matching ``_quantize_k_cache_ref`` /
    ``_quantize_k_cache_fast_kernel``) to keep CPU and GPU KV cache bytes
